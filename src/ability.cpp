...
Ability::Ability(EffectValue effect)
	: Ability(effect, std::make_shared<const NoTargets>())
{ }
...
{
	Changeset changes;
	changes.push_back(AddMana{ env.getController(s), mana });
	return changes;
}),
...
Changeset ManaAbility::applyEffect(const Environment&) const {
	Changeset changes;
	changes.push_back(AddMana{ this->owner, mana });
	return changes;
}