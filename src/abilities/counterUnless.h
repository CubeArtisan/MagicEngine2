		...
		: clone_inherit(LambdaEffects([this](xg::Guid s, const Environment& env) -> std::optional<Changeset>
			{
				...
				if (std::dynamic_pointer_cast<Card>(target)) {
					counter.moves.push_back(ObjectMovement{ target->id, env.stack->id, env.graveyards.at(target->owner)->id, 0, COUNTER });
				}
				else {
					counter.remove.push_back(RemoveObject{ target->id, env.stack->id });
				}
