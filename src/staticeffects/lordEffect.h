...
public:
	StaticEffectQuery& handleEvent(StaticEffectQuery& query, const Environment& env) const override {
		if (PowerQuery* ptr = std::get_if<PowerQuery>(&query)) {
			if (this->powerBoost != 0 && (this->owner != ptr->target.id || this->includeOwner) && prop(ptr->target, env) && env.battlefield->findObject(ptr->target.id)) {
				ptr->power += this->powerBoost;
				return query;
			}
		}
		if (ToughnessQuery* ptr = std::get_if<ToughnessQuery>(&query)) {
			if (this->toughnessBoost != 0 && (this->owner != ptr->target.id || this->includeOwner) && prop(ptr->target, env) && env.battlefield->findObject(ptr->target.id)) {
				ptr->toughness += this->toughnessBoost;
				return query;
			}
		}
		if (StaticEffectsQuery* ptr = std::get_if<StaticEffectsQuery>(&query)) {
			if (!this->staticEffectBoost.empty() && (this->owner != ptr->target.id || this->includeOwner) && prop(ptr->target, env) && env.battlefield->findObject(ptr->target.id)) {
		...
		if (ReplacementEffectsQuery* ptr = std::get_if<ReplacementEffectsQuery>(&query)) {
			if (!this->replacementEffectBoost.empty() && (this->owner != ptr->target.id || this->includeOwner) && prop(ptr->target, env) && env.battlefield->findObject(ptr->target.id)) {
		...
		if (TriggerEffectsQuery* ptr = std::get_if<TriggerEffectsQuery>(&query)) {
			if (!this->triggerEffectBoost.empty() && (this->owner != ptr->target.id || this->includeOwner) && prop(ptr->target, env) && env.battlefield->findObject(ptr->target.id)) {
	...
	bool dependsOn(StaticEffectQuery&, StaticEffectQuery&, const Environment&) const override {
		// CodeReview if prop would change from start to end
		return false;
	}

	LordEffect(PropositionValue<CardToken, Environment> prop, int powerBoost=0, int toughnessBoost=0,
		...
		: clone_inherit({}, { BATTLEFIELD }), prop(prop), powerBoost(powerBoost), toughnessBoost(toughnessBoost),
...
private:
	PropositionValue<CardToken, Environment>  prop;
	int powerBoost;
	int toughnessBoost;
